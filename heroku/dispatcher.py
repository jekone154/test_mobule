    ...
    async def command_exc(self, _, message: Message):
        ...
        if isinstance(exc, RPCError):
            if isinstance(exc, FloodWaitError):
                ...
                _trans = self._client.loader.lookup("translations")
                if _trans:
                    txt = _trans.strings("fw_error").format(
                        utils.escape_html(message.message),
                        fw_time,
                        type(exc.request).__name__,
                    )
                else:
                    txt = f"⏳ FloodWait {fw_time} on {utils.escape_html(message.message)}" 
            else:
                txt = (
                ...
                _trans = self._client.loader.lookup("translations")
                if _trans:
                    txt = _trans.strings("rpc_error").format(
                        utils.escape_html(message.message),
                        utils.escape_html(str(exc)),
                    )
                else:
                    txt = f"🚫 RPC error on {utils.escape_html(message.message)}: {utils.escape_html(str(exc))}" 
        else:
