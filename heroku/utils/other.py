...
async def invite_inline_bot(
    ...
    bot_username = getattr(getattr(getattr(client, "loader", None), "inline", None), "bot_username", None)
    if not bot_username:
        logger.warning("invite_inline_bot: inline bot not ready yet, skipping invite")
        return
    ...
    try:
        ...
        if "already" in err or "user_already" in err:
            ...
            logger.warning("invite_inline_bot: %s", e)
