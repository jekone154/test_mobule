...
class HerokuBackupMod(loader.Module):
    ...
    async def client_ready(self):
        ...
        # Не зависаем вечно — просто берём что есть сейчас
        self._content_channel_id = self._db.get("heroku.forums", "channel_id", None)
    ...
    async def handler(self):
        try:
            ...
            sleep_time = self.get("last_backup") + self.get("period") - time.time()
            if sleep_time > 0:
                await asyncio.sleep(sleep_time)

            # Обновляем channel_id если не был загружен при старте
            if not getattr(self, "_content_channel_id", None):
                self._content_channel_id = self._db.get("heroku.forums", "channel_id", None)

            if not self._content_channel_id:
                logger.warning("HerokuBackup: content channel not set up yet, skipping backup")
                await asyncio.sleep(60)
                return

            backup_topic_id = await utils.get_topic_id(self._db, "Backups")
            if not backup_topic_id:
                logger.warning("HerokuBackup: Backups topic not found, skipping backup")
                await asyncio.sleep(60)
                return
    ...
    async def restore(self, call: BotInlineCall):
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                if _current_token:
                    if "heroku.inline" not in self._db:
                        self._db["heroku.inline"] = {}
                    self._db.set("heroku.inline", "bot_token", _current_token)
    ...
    async def backupdb(self, message: Message):
        ...
        if not self._content_channel_id:
            await utils.answer(message, "<b>❌ Контент-канал не настроен. Запусти quickstart.</b>")
            return

        backup_topic_id = await utils.get_topic_id(self._db, "Backups")
        if not backup_topic_id:
            await utils.answer(message, "<b>❌ Топик Backups не найден. Запусти quickstart.</b>")
            return
