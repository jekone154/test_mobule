    ...
    **kwargs,
    ...
    if reply_markup is not None:
        ...
        if reply_markup:
            ...
            _inline = getattr(getattr(getattr(message, "client", None), "loader", None), "inline", None)
            if not _inline or not _inline.init_complete:
                await message.edit(response)
                return
            reply_markup = _inline._normalize_markup(reply_markup)
            result = await _inline.form(
                response,
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "heroku_grepped"):
            try:
                ...
                _inline2 = getattr(getattr(getattr(message, "client", None), "loader", None), "inline", None)
                if not _inline2 or not _inline2.init_complete:
                    await message.edit(response)
                    return
                list_ = await _inline2.list(
                    message=message,
                    strings=strings,
                )
